        ...
        <div className="space-y-4">
          ...
          {team.length === 0 ? (
            ...
            team.map((member) => {
              ...
              const isOnLeave = submission?.status === 'On - Leave';
              ...
              return (
                <motion.div 
                  ...
                  key={member.user_id}
                  className={cn(
                    "group p-3 bg-white rounded-2xl border border-slate-50 border-l-4 shadow-sm flex items-center justify-between transition-all",
                    isOnLeave ? "border-amber-400" : isCompletedByManager ? "border-green-400" : "border-slate-50",
                    !(member.role.toUpperCase() === 'RM' || member.role.toUpperCase() === 'SM') && "hover:shadow-md",
                    isCompletedByManager && (isOnLeave ? "bg-amber-50/20" : "bg-slate-50/25")
                  )}
                >
                  <div 
                    className={cn(
                  ...
                  <div className="flex items-center gap-2">
                    {viewedUser.user_id === user.user_id ? (() => {
                      ...
                      return (
                        ...
                        >
                          {!hasQuestions ? 'No Questions' : isOnLeave ? 'Edit On-Leave' : isCompletedByManager ? 'Edit Update' : 'Submit Update'}
                        </button>
                      );
                    })() : (
                      <div className={cn(
                        "flex flex-col items-center justify-center w-[65px] h-[52px] rounded-xl text-center leading-tight",
                        isOnLeave ? "bg-amber-50 text-amber-600" : isCompletedByManager ? "bg-green-50 text-green-600" : "bg-slate-50 text-slate-400"
                      )}>
                        <p className="text-[8px] font-black uppercase tracking-tighter">Daily Update</p>
                        <p className="text-[9px] font-bold">{isOnLeave ? 'LEAVE' : isCompletedByManager ? 'DONE' : 'PENDING'}</p>
                      </div>
                    )}
                  </div>
                </motion.div>
